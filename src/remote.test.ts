...
import { ObjectStore, SET, type RemoteAble, type RemoteAwaited, type RemoteReadonly } from "./index.js";
...
describe("Remote<T>", () => {
  ...
  test("get functions", async () => {
    const i = new (class Test {
      ...
      h: ((value: number) => number) | ((value: number, second: string) => Promise<string>) = () => 1;
    })();
    const r = R(i);
    let test = 0;
    // Working
    ...
    const h = (await r.h) satisfies ((value: number) => PromiseLike<number>) | ((value: number, second: string) => PromiseLike<string>);
    expect((await h(2, "TT")) satisfies number | string).toBe(1);
  });
  test("get classes", async () => {
    ...
    let test = 0;
    // Working
    const a = (await r.A) satisfies RemoteAwaited<typeof A>;
    expect((await new a()) satisfies RemoteAwaited<A>).toBeInstanceOf(a);
    const b = (await r.B) satisfies RemoteAwaited<typeof B>;
    const bi = (await new b(2)) satisfies RemoteAwaited<B>;
    expect(bi).toBeInstanceOf(b);
    expect((await bi.num) satisfies number).toBe(2);
    expect(test).toBe(2);
    // working but types should fail
    const a1 = (await r.A) satisfies RemoteAwaited<typeof A>;
    // @ts-expect-error
    expect((await new a1()) satisfies RemoteAwaited<B>).toBeInstanceOf(a);
  });
  ...
  test("set functions", async () => {
    const i = new (class Test {
      ...
      h: ((value: number) => Promise<number>) | ((value: number, second: string) => Promise<string>) = async () => 1;
    })();
    const r = R(i);
    // Working
    let test = 0;
    ...
    expect(test).toBe(8);
    await r.h[SET]((v: number) => v);
    expect((await r.h(2, "TT")) satisfies number | string).toBe(2);
    await r.h[SET](async (v: number, s: string) => v + s);
    expect((await r.h(2, "TT")) satisfies number | string).toBe("2TT");
    // Working but types should fail
    test = 0;
    // @ts-expect-error
  ...
  test("calling functions with function values", async () => {
    ...
    expect((await r.b(r.bt)) satisfies number).toBe(4);
    const cafn = async () => {};
    ...
    // @ts-expect-error
    expect((await r.a(async (_a: string) => 2)) satisfies number).toBe(2);
    // @ts-expect-error
    expect((await r.a((_a: string) => 2)) satisfies number).toBe(2);
    const cafnf = async (_a: string) => {};
    // @ts-expect-error
    expect((await r.c(cafnf)) satisfies () => void).toBe(cafnf);
    const cfnf = (_a: string) => {};
    // @ts-expect-error
    expect((await r.c(cfnf)) satisfies () => void).toBe(cfnf);
    const dfnr = (_v: number) => "test";
    // @ts-expect-error
    expect((await r.d(dfnr)) satisfies number).toBe("test");
    const dfnp = (_v: string) => 4;
    // @ts-expect-error
    expect((await r.d(dfnp)) satisfies number).toBe(4);
  });
  test("calling functions with class values", async () => {
    class Base {
      a: number;
    ...
    const i = new (class Test {
      ...
      async a(constructor: new (a: number) => Base): Promise<Base> {
        return new constructor(10);
      }
      async b(constructor: new (a: number) => PromiseLike<Base>): Promise<Base> {
        return await new constructor(10);
      }
    })();
    const r = R(i);
    // working
    const a = (await r.a(r.RemoteClass)) satisfies RemoteAwaited<Base>;
    expect((await a.a) satisfies number).toBe(10);
    expect(a).toBeInstanceOf(await r.RemoteClass);
    const b1 = (await r.b(r.RemoteClass)) satisfies RemoteAwaited<Base>;
    expect((await b1.a) satisfies number).toBe(10);
    expect(b1).toBeInstanceOf(await r.RemoteClass);
    const b2 = (await r.b(LocalClass)) satisfies RemoteAwaited<Base>;
    expect((await b2.a) satisfies number).toBe(10);
    expect(b2).toBeInstanceOf(LocalClass);
    // working but types should fail
    // @ts-expect-error
    const a1 = (await r.a(LocalClass)) satisfies RemoteAwaited<Base>;
    expect((await a1.a) satisfies number).toBe(10);
    expect(a1).toBeInstanceOf(LocalClass);
  });
  test("constructing instances", async () => {
    class A {
      constructor() {
        test++;
      }
    ...
    let test = 0;
    // working
    const ai = (await (new r.A() satisfies RemoteReadonly<A>)) satisfies RemoteAwaited<A>;
    expect(ai instanceof (await r.A)).toBe(true);
    expect(test).toBe(1);
  });
});
